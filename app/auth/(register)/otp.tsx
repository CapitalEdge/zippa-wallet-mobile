...
import { Container, theme, Text } from '../../../components/Themed'
...
export default function OTPScreen() {
    ...
    const __handleOTPVerification = async () => {
        ...
        if (emailAddress !== null) {
            ...
            //remove the previously stored local value of email
            MMKVStorage.removeItem('@email');
            if (verifyError === null) {
                setAppState('IDLE');
                router.replace('/auth/(register)/set-transaction-pin');
            } else {
                setAppState('IDLE');
    ...
    const __resendOtpVerification = async () => {
        setAppState("LOADING")
        await resendOTP(emailAddress)
        setAppState("IDLE");
        await __timer();
    }
            ...
            <Container>
                <OTPContent>
                    ...
                    <OTPTextContainer>
                        ...
                        {time === 0 ? <Pressable onPress={__resendOtpVerification}>
...
const OTPButton = styled.Pressable`
  width: 100%;
  height: 50px;
