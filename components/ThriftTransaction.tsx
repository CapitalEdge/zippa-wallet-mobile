...
export default function ThriftTransactions({ data }: any): JSX.Element {
    return (
                ...
                <TransactionItem key={item?.node?.id}>
                    <Icon>
                        <IconSuccessContainer>
                            <Entypo name="arrow-long-up" size={18} color={theme.colors.zippaOrange} />
                        </IconSuccessContainer>
                    </Icon>
                    <Details>
                        <DateAndTitle>
                            <View style={{ width: '65%' }}>
                                <Text fontSize={14}  style={{ textTransform: "capitalize" }}>Thrift savings top up</Text>
                            </View>
                            <View style={{ width: '35%', position: 'absolute', top: 20, right: -15 }}>
                                <Text fontFamily="zippa-semibold" fontSize={16} color={theme.colors.zippaOrange}>+₦{parseFloat(item?.node?.amount).toLocaleString('en')}</Text>
                            </View>
                        </DateAndTitle>
...
const IconFailureContainer = styled(IconSuccessContainer)`
    background-color: ${theme.colors.zippaSubtleRed};`;
